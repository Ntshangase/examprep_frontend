...
import { getGeneratedTest, submitAnswers } from "../../../Api/Api";
...
const IndStudentWriteTest = () => {
	...
	console.log("paramTestId from useParams:", paramTestId); // Debug: check raw URL parameter
 
	// Attempt to parse testId, defaulting to null if invalid
	const testId = paramTestId && !isNaN(parseInt(paramTestId, 10)) ? parseInt(paramTestId, 10) : null;
	console.log("Parsed testId:", testId); // Debug: ensure testId is a number
	...
	useEffect(() => {
		...
		console.log("Test ID type:", typeof testId); // Check if testId is a number
	...
	console.log("The test Id for this test is ",testId);
	...
	const handleAnswerChange = (questionId, answerId) => {
		...
		newAnswers.push({ questionId, answerId,selected:true }); // Add the new answer
	...
	const handleSubmit = async () => {
		...
		try{
			await submitAnswers(testId,user.id, answers);
		}catch(error){
			console.log(error);
		}
	};
